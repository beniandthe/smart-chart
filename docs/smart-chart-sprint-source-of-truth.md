...
- `EditorView.swift` no longer owns chord confirmation/correction sheet UI, rhythm confirmation sheet UI, or the shared flow layout after Sprint 19, but it remains broad at roughly `1544` lines. `LeadSheetCanvasHostView.swift` no longer owns chord edit overlay geometry, measure resize handle geometry, active ink-scope support, or saved ink image rendering after Sprint 23, but it remains the largest live editor bridge at roughly `1403` lines.
...
### Sprint 24: Editor Bridge Cleanup Gate

Status: queued after Sprint 23 GitHub verification.
...
Candidate Sprint 24 directions:

- Continue `LeadSheetCanvasHostView.swift` cleanup with a low-risk extraction such as gesture routing helpers, ink persistence support, or chord recognition scheduling support.
...
- next sprint candidate: Sprint 24 editor bridge cleanup gate after Sprint 23 GitHub checks pass.

## Next Sprint Backlog

Use this queue for Sprint 24 after Sprint 23 GitHub checks pass. The user has approved continuing through the current audit/cleanup plan one scoped sprint at a time until a necessary approval/input point or plan completion.

- Continue editor surface cleanup with one behavior-preserving extraction from `LeadSheetCanvasHostView.swift`, likely gesture routing helpers, ink persistence support, or chord recognition scheduling support.
