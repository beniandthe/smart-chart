...
- The V1 readiness matrix is open at `docs/smart-chart-v1-readiness-matrix-2026-05-31.md`.
- The first audit-found gap was closed with a repository-level persistence proof for a V1-shaped Simple Chord Sheet containing role typography, a manual row break, manual width, roadmap objects, cue text, measure-attached chart-area freehand, and rendered chords.
- `git diff --check` passed; focused SwiftPM audit group passed with `165` tests and `0` failures; full `swift test --scratch-path /tmp/SmartChartSwiftBuild-layoutprofile` passed with `438` tests, `36` skipped, and `0` failures; XcodeBuildMCP `build_run_sim CODE_SIGNING_ALLOWED=NO` succeeded on the configured iPad Pro 13-inch simulator with the existing headermap warning only; screenshot capture showed the app launched to Projects.
- The remaining release gates are fresh app-surface Simple and Rhythm Section save/reopen/export live passes.
