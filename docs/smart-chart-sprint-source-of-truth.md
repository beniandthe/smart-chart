...
### Sprint 23: Editor Bridge Cleanup Gate

Status: queued after Sprint 22 GitHub verification.
...
Candidate Sprint 23 directions:

- Continue `LeadSheetCanvasHostView.swift` cleanup with a low-risk extraction such as saved ink rendering helpers, gesture routing helpers, or ink persistence support.
...
- next sprint candidate: Sprint 23 editor bridge cleanup gate after Sprint 22 GitHub checks pass.

## Next Sprint Backlog

Use this queue for Sprint 23 after Sprint 22 GitHub checks pass. The user has approved continuing through the current audit/cleanup plan one scoped sprint at a time until a necessary approval/input point or plan completion.

- Continue editor surface cleanup with one behavior-preserving extraction from `LeadSheetCanvasHostView.swift`, likely saved ink rendering helpers, gesture routing helpers, or ink persistence support.
