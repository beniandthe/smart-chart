...
- latest Simple automatic chord placement/no-resize correction: Simple Chord Sheet chord editing no longer shows a separate move/resize-looking control. The selected chord body is the move target, delete remains the only visible chord edit control, and Simple append placement ignores handwriting fraction so the first chord in an empty measure lands on beat `1` and the second chord lands on beat `3` in common time. Moving an existing chord still snaps to the dragged beat, while rendered chord size stays automatic from the measure, beat segment, and text fit. Focused SwiftPM `ChartEditingTests` plus `LeadSheetPageLayoutTests` passed with `146` tests and `0` failures; focused simulator `ChartEditingTests`, `LeadSheetChordEditOverlayGeometryTests`, `LeadSheetInteractionModeStatePolicyTests`, and Simple chord layout cases passed with `34` tests and `0` failures; full `swift test --scratch-path /tmp/SmartChartSwiftBuild-layoutprofile` passed with `422` tests, `36` skipped, and `0` failures.
