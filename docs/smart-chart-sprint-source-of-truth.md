...
- `EditorView.swift` no longer owns chord confirmation/correction sheet UI, rhythm confirmation sheet UI, or the shared flow layout after Sprint 19, but it remains broad at roughly `1544` lines. `LeadSheetCanvasHostView.swift` no longer owns chord edit overlay geometry, measure resize handle geometry, active ink-scope support, saved ink image rendering, active ink persistence write-back decisions, or chord ink bounds/OCR image rendering after Sprint 25, but it remains the largest live editor bridge at roughly `1352` lines.
...
### Sprint 26: Editor Bridge Cleanup Gate

Status: queued after Sprint 25 GitHub verification.
...
Candidate Sprint 26 directions:

- Continue `LeadSheetCanvasHostView.swift` cleanup with a low-risk extraction such as gesture routing helpers, chord recognition scheduling support, or rhythm finalization support.
...
Use this queue for Sprint 26 after Sprint 25 GitHub checks pass. The user has approved continuing through the current audit/cleanup plan one scoped sprint at a time until a necessary approval/input point or plan completion.

- Continue editor surface cleanup with one behavior-preserving extraction from `LeadSheetCanvasHostView.swift`, likely gesture routing helpers, chord recognition scheduling support, or rhythm finalization support.
