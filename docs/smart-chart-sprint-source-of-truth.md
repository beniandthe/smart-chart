...
- The follow-up pass confirmed `Absus` recognition is much better; the remaining issue is live chord-ink visual persistence for small `s` curves, not early render timing.
...
- Chord-entry ink should use a solid visual tool that preserves small `s` curves as written; do not change recognition scheduling for this issue.
...
- Chord-entry ink uses a more visually persistent PencilKit tool without changing recognition scores, OCR, correction memory, or scheduling.
