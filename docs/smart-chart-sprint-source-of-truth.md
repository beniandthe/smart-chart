...
- Chord-entry ink should keep the live chord-ink chart snapshot synchronized while writing, so refreshes do not overwrite active `PKCanvasView` strokes with stale model data.
...
- Chord-entry ink persists live writing into the chart snapshot without changing the pen tool, stroke weight, recognition scores, OCR, correction memory, or scheduling.
