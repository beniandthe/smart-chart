...
- `EditorView.swift` no longer owns chord confirmation/correction sheet UI, rhythm confirmation sheet UI, or the shared flow layout after Sprint 19, but it remains broad at roughly `1544` lines. `LeadSheetCanvasHostView.swift` remains the largest live editor bridge at roughly `1622` lines.
...
### Sprint 20: Editor Bridge Cleanup Gate

Status: queued after Sprint 19 GitHub verification.

Goal: continue the approved cleanup/audit plan with one behavior-preserving editor extraction, likely from `LeadSheetCanvasHostView.swift` or another remaining `EditorView.swift` modal surface.
...
Candidate Sprint 20 directions:

- Inspect `LeadSheetCanvasHostView.swift` and choose one low-risk extraction that does not affect native `PKCanvasView` writing feel, recognition timing, ink lifecycle, or layout behavior.
...
- next sprint candidate: Sprint 20 editor bridge cleanup gate after Sprint 19 GitHub checks pass.

## Next Sprint Backlog

Use this queue for Sprint 20 after Sprint 19 GitHub checks pass. The user has approved continuing through the current audit/cleanup plan one scoped sprint at a time until a necessary approval/input point or plan completion.

- Continue editor surface cleanup with one behavior-preserving extraction from `LeadSheetCanvasHostView.swift`, likely focusing on a contained helper/view boundary that preserves native `PKCanvasView` behavior.
