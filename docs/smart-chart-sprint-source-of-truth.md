...
- Latest Simple chord-fit pass is implemented locally: Simple chord layout now gives rendered chords the full snapped measure segment instead of a text-width box, so a single downbeat chord spans the cell and multiple chords divide the measure by snapped attack positions. Simple chord font sizing is style-independent and scales into that segment, shrinking only when a longer symbol would overflow.
